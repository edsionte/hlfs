...
int hlfs_write(struct hlfs_ctrl *ctrl, char *write_buf, uint32_t write_len, uint64_t pos)
{
	...
	//初始化miniLZO的库
	if (init_compress() != 0) {
		printf("init_compress error..\n");
		goto out;
	}

	//将上述的datablocks以块大小为单位进行压缩，并将压缩长度附加到压缩数据之前
	/*
	 *压缩后数据的组织如下：
	...
	uint32_t outlen = 0;
	char *compressed_db = g_malloc0((db_end - db_start + 1) * BLOCKSIZE);
	...
	for (cur_dbno = db_start; cur_dbno <= db_end; cur_dbno++) {
		...
		memcpy(compressed_db + compressed_len, &outlen, sizeof(uint32_t));
		compressed_len += sizeof(uint32_t);
		memcpy(compressed_db + compressed_len, outdata, outlen);
		compressed_len += outlen;

	}
	*(uint32_t *)compressed_db = compressed_len;
	*(uint32_t *)(compressed_db + sizeof(uint32_t)) = ib_amount(db_start, db_end);
...
write_log:;
	  ...
	  if(ctrl->cctrl != NULL){
		  ...
		  int size = append_log(ctrl, compressed_db, db_start, db_end);
		  g_mutex_unlock  (ctrl->hlfs_access_mutex);
		  if(size < 0){
